...
function calcROI() {
	...
	Promise.all([requestDataOne, requestDataTwo]).then((messages) => {
		...
		currentPrice = bitcoin_amt * usdPrice;
		...
		document.getElementById("r2-c1").innerHTML = "Price of BTC on " + date_searched;
		...
		document.getElementById("head").innerHTML = bitcoin_amt + "BTC" + " | " + currentPrice + "$" + " | " + "ROI" + percent_change + '%';
	})
}
